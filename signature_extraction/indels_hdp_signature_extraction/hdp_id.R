# HDP indel signature extraction by node.
...
# read in the data
tnt <- read.csv("ID_category_counts.txt", sep="\t", header = T, stringsAsFactors =F, row.names = 1)
patients <- sapply(strsplit(rownames(tnt), ":"), "[[", 1)

# read in PCAWG sigs
sigs <- read.csv("PCAWG_sigProfiler_ID_signatures.csv", stringsAsFactors = F)

# I only want to keep a subset of them.
gdsigs <- c("ID1", "ID2", "ID3", "ID4", "ID5", "ID6", "ID7", "ID8", "ID10","ID14") 
sigs <- as.matrix(sigs[,gdsigs])

# initially had 1000 weighting for each, but for this run am increasing 
prior_pseudoc <- rep(20000, ncol(sigs)) # this is meant to be a vector of pseudocounts contributed by each prior distribution.
...
test_chlist <- hdp_posterior(test_pr, burnin=50000, n=200, space=2000, cpiter=3, seed=randseed)

# save data.
assign(paste0("hdp_id_", jobIndex), test_chlist)

save(list=paste0("hdp_id_", jobIndex), file=paste0("id_out_", jobIndex, ".RData"))
